...
module main() {
	difference() {
		union() {
			...
			difference() {
				// Base wheel.
				union(){cylinder(r=wheel_radius,h=wheel_height,$fn=100);
                    outerd=10; //was 10
                    Width=1;
                 translate([0,0,wheel_height+Width/2]){cylinder( h=Width, r=outerd/2, center=true, $fn=40 );} //thickness for screw   
                }
				// Cut to angle.
...
module servomount(counter_sink_height=1.5, counter_sink_diameter=6.0, num_teeth=21) {
	...
	//countersink
    translate([0,0,.99]){cylinder(r1=outer_diameter/2,r2=0,h=outer_diameter/2,$fn=100);}
	//hole for screw
    cylinder(r=screw_diameter/2,h=spline_height+0.9+counter_sink_height,$fn=100);
    //space for head of screw
	//translate([0,0,spline_height+0.9])
	//cylinder(r=counter_sink_diameter/2,h=counter_sink_height,$fn=100);
    //add clearance for servo translate+h/2
    //include clearance for screw head r=22/2 -> r=24/2
    translate([0,0,0.49]){cylinder( h=1, r=24/2, center=true, $fn=40 );}
